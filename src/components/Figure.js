...
import { incrementError, decrementError, resetError } from '../store/error'
...
const Figure = () => {
  ...
  const handleIncrementError = (e) => {
    e.preventDefault()
    dispatch(incrementError())
  }

  const handleDecrementError = (e) => {
    e.preventDefault()
    dispatch(decrementError())
  }
  ...
  return (
    <>
    <svg className="hangman-figure mt-5 rounded-lg bg-primary-content" height="300" width="400">
      <g id="body">
        {error > 1 && <g id="head">
    ...
    <button className='p-5' onClick={handleIncrementError}>Increment</button>
    <button className='p-5' onClick={handleDecrementError}>Decrement</button>
    <button className='p-5' onClick={handleResetError}>Reset</button>
    </>
